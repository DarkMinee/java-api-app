...
type StudentFormDefaults = Pick<NewStudent, 'id' | 'studentTasks'>;

type StudentFormGroupContent = {
  id: FormControl<IStudent['id'] | NewStudent['id']>;
  name: FormControl<IStudent['name']>;
  surname: FormControl<IStudent['surname']>;
  date: FormControl<IStudent['date']>;
  classroom: FormControl<IStudent['classroom']>;
  studentTasks: FormControl<IStudent['studentTasks']>;
};
...
export class StudentFormService {
  createStudentFormGroup(student: StudentFormGroupInput = { id: null }): StudentFormGroup {
    ...
    return new FormGroup<StudentFormGroupContent>({
      ...
      surname: new FormControl(studentRawValue.surname),
      date: new FormControl(studentRawValue.date),
      classroom: new FormControl(studentRawValue.classroom),
      studentTasks: new FormControl(studentRawValue.studentTasks ?? []),
    });
  }
